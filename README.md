...
To illustrate the specific data format, we provide a small dataset of Schubert's lieder compositions from the [OpenScore Lieder](https://github.com/OpenScore/Lieder), which includes interleaved ABC folders, augmented ABC folders, as well as data index files for training and evaluation. You can download it here and put it under ```data/```.
...
Here we give an example on fine-tuning with the Schubert's lieder data mentioned above:
