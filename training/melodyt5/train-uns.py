...
class MelodyHubDataset(Dataset):
    def __init__(self, items):
        ...
        for item in tqdm(items):
            tune =  patchilizer.encode(item['output'], add_special_patches=True)
            tune = torch.tensor(tune)
    ...
    def __getitem__(self, idx):
        output_patch = self.tunes[idx]

        input_patch = output_patch.clone().detach()
        num_noise = torch.randint(1, len(input_patch), (1,)).item()
        if len(input_patch) > 1:
            noise_idx = torch.randperm(len(input_patch))[:num_noise]
            input_patch[noise_idx] = torch.tensor([patchilizer.mask_token_id] * PATCH_SIZE)
...
if __name__ == "__main__":
    ...
    # load data
    # with open('new_wikimt.json') as f:
    #     print("Loading Data...")
    #     data = json.load(f)
