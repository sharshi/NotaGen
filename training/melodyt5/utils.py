...
class PatchLevelEnDecoder(PreTrainedModel):
    ...
    def forward(self,
        ...
        if masks==None and decoder_masks==None:
...
class MelodyT5ForClassification(PreTrainedModel):
    """
    A wrapper for the MelodyT5 model for classification tasks.
    """
    def __init__(self, encoder_config, num_classes):
        ...
        self.patch_level_decoder = PatchLevelEnDecoder(encoder_config)
        self.classifier = torch.nn.Linear(encoder_config.n_embd, num_classes)
    ...
    def forward(self,
        ...
        encoded_patches = self.patch_level_decoder(patches=patches, 
                                                   masks=None,
                                                   decoder_patches=patches.clone().detach(),
                                                   decoder_masks=None)
