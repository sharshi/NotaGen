...
def get_args(parser):
    ...
    parser.add_argument('-num_tunes', type=int, default=100, help='the number of independently computed returned tunes')
    parser.add_argument('-max_patch', type=int, default=256, help='integer to define the maximum length in tokens of each tune')
...
def generate_abc(args):
    ...
    with open('prompt_tunesformer.txt', 'r') as f:
        prompt = f.read()
    ...
    for i in range(num_tunes):
        ...
        if PATCH_MODE == 'bar':
            ...
            tokens = torch.tensor([ord(c) for c in remaining_tokens], dtype=torch.int, device=device)
        
        while input_patches.shape[1]<max_patch:
            # print('input_patches\n', patchilizer.decode(input_patches[0]))
            # print('tokens\n', patchilizer.decode(tokens))
            ...
            if predicted_patch[0]!=patchilizer.eos_token_id:
                ...
                if PATCH_MODE == 'bar':
                    ...
                    predicted_patch = torch.tensor(patchilizer.bytes2patches(next_bar), device=device).unsqueeze(0)
