...
from transformers import LlamaConfig, GPT2Config, get_scheduler, get_constant_schedule_with_warmup
...
char_Llama_config = LlamaConfig(num_hidden_layers=CHAR_NUM_LAYERS,
                          max_position_embeddings=PATCH_SIZE,
                          vocab_size=128,
                          hidden_size=768,
                          num_attention_heads=12,)
                        #   _attn_implementation='eager')
char_GPT2_config = GPT2Config(num_hidden_layers=CHAR_NUM_LAYERS,
...
model = TunesFormer(patch_config, char_Llama_config, share_weights=SHARE_WEIGHTS)
...
scaler = GradScaler()
is_autocast = True
optimizer = torch.optim.AdamW(model.parameters(), lr=LEARNING_RATE, betas=(0.9, 0.95), eps=1e-5)
    ...
    def __init__(self, items):
        ...
        for item in tqdm(items):
            ...
            if torch.sum(output_patch)!=0:
                self.texts.append(output_patch)
