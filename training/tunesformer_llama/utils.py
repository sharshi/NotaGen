...
from transformers import LlamaModel, LlamaForCausalLM, PreTrainedModel
...
class CharLevelDecoderLlama(PreTrainedModel):
    """
...
class CharLevelDecoderGPT2(PreTrainedModel):
    """
    A Char-level Decoder model for generating the characters within each bar patch sequentially.
    It inherits PreTrainedModel from transformers.
    """
    ...
    def forward(self, encoded_patches: torch.Tensor, target_patches: torch.Tensor, patch_sampling_batch_size: int):
        ...
        results = self.base(inputs_embeds=inputs_embeds,
                         attention_mask=target_masks,
                         labels=labels)
...
class TunesFormer(PreTrainedModel):
    ...
    def __init__(self, encoder_config, decoder_config, share_weights=False):
        ...
        # self.char_level_decoder = CharLevelDecoderGPT2(decoder_config)
        self.char_level_decoder = CharLevelDecoderLlama(decoder_config)
